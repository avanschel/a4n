...
export async function importData(db,server,dispatch,state){
    state = {loading:true,button:false,error:true,errorMessage:'Error occured while creating structure.',cat:'IMPORT IN PROGRESS',text:'Init storage ',percent:5};
    ...
    let importData = new Promise(resolve=>{
        ...
        createAfmFldsTable(db).then(res=>{
            if(res){
                // Step 2 and 3
                state = {loading:true,button:false,error:false,errorMessage:null,cat:'IMPORT IN PROGRESS',text:'Init storage ',percent:5};
                ...
                insertAfmFieldInAfmTbl(db,false).then(iAFA=>{
                    if(res.error){
                        ...
                        state = {loading:true,button:false,error:false,errorMessage:null,cat:'IMPORT IN PROGRESS',text:'Retrieve assets definitions ',percent:10};
                        dispatch({type:PROGRESS_UPDATE,state:state});
                        // Step 4 and 5
                        retrieveAllTableDefinitions(db,dispatch,server,state).then(fieldDefinitions=>{
                            state = {loading:true,button:false,error:false,errorMessage:null,cat:'IMPORT IN PROGRESS',text:'Creating table in storage ',percent:50};
                            dispatch({type:PROGRESS_UPDATE,state:state});
                            // Step 6
                            createAllTables(db,dispatch,state).then(tableDefs=>{
                                // Step 7
                                state = {loading:true,button:false,error:false,errorMessage:null,cat:'IMPORT IN PROGRESS',text:'Retrieving data from server ',percent:70};
                                dispatch({type:PROGRESS_UPDATE,state:state});
                                //FINAL STEP HERE
    ...
    return importData.then(res=>{
        state = {loading:true,button:true,error:false,errorMessage:null,cat:'IMPORT IN PROGRESS',text:'import finished successfully',percent:100};
        dispatch({type:PROGRESS_UPDATE,state:state});
        return{type:PROGRESS_UPDATE,state:state};
    }).catch(error=>{
        state = {loading:true,button:true,error:true,errorMessage:error,cat:'IMPORT IN PROGRESS',text:'import finished successfully',percent:100};
        dispatch({type:PROGRESS_UPDATE,state:state});
...
export async function insertAfmFldsAssetDef(db,dispatch,state,data) {
    ...
    for (let def of data) {
        ...
        state = {loading:true,button:false,error:false,errorMessage:null,cat:'IMPORT IN PROGRESS',text:'Inserting def '+def.table_name+'.'+def.field_name+' ',percent:percent};
        dispatch({type:PROGRESS_UPDATE,state:state});
    }
    return state;
}
...
export async function launchCreateTable(db,dispatch,state,data){
    ...
    for(let i=0;i<data.length;i++){
        ...
        await new Promise(resolve=>{
            state = {loading:true,button:false,error:false,errorMessage:null,cat:'IMPORT IN PROGRESS',text:'Create table '+data[i].table_name+' ',percent:percent};
            dispatch({type:PROGRESS_UPDATE,state:state});

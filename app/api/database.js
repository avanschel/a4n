...
export async function eraseTableAfmFlds(db,tbl=false) {
    db.transaction(tx => {
        ...
        },(err) => {return {error:true,message:err}},
    );
    return {error:false,message:''};
}
...
export async function insertAssetAfmFlds(db,tbl_name,fields,values){
    return new Promise(resolve=>{
        db.transaction(tx=> {
            ...
            (error)=>{resolve({error:true,message:error});}
        );
    });
}
    ...
    return new Promise(resolve=>{
        db.transaction(tx => {
            tx.executeSql('drop table if exists '+tblName,[],
                (transact,result)=>{
                    tx.executeSql(
                        `select * from afm_flds where table_name like ? order by primary_key asc`,  [tblName.replace(SURV_PREFIX,'')],
                        (_, { rows: { _array } }) => {
                            let columnString ="";
                                    ...
                                    _array.map((item,ind)=>{
                                        if(item.primary_key.length>0 || item.scantofld>0){
                                            tx.executeSql("create index idx_"+tblName+"_"+item.field_name+" on "+tblName+"("+item.field_name+")",[],
                                                ...
                                                },(f,er)=>{resolve({error:true,message:er});});
                                        }
                                    })
                                },(tr,err)=>{
                                    resolve({error:true,message:err});

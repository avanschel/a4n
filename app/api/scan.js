...
export async function retrieveDataForCodeBar(db,dispatch,state,codeBar){
    let data;
    let nextState;
    let status=SCAN_NOT_FOUND;
    return new Promise(resolve=>{
        if(codeBar === null){
            nextState = {loading:true,error:false,text:'',container:state.container,code:null,founded:false,status:false,data:{},defFields:state.defFields,survey:{},canScan:true};
            resolve(setNewCodeBar(nextState));
        }else{
        asyncRetrieveAssetsById(db,'ta','barcode',codeBar,null).then(res=>{
            if(res.length){
                data =res[0];
                ...
                nextState = {...state,loading:false,error:false,text:'',code:codeBar,founded:true,status:status,data:data,survey:data,original:{bl_id:data.bl_id,fl_id:data.fl_id,rm_id:data.rm_id}}
            }else{
                //On genere un objet vide
                let surv = {};
                state.defFields.map((defField)=>{
                    surv[defField.field_name]=(defField.field_name==='barcode')?codeBar:(defField.field_name==='bl_id')?state.container.bl_id:(defField.field_name==='fl_id')?state.container.fl_id:(defField.field_name==='rm_id')?state.container.rm_id:'';
                });
                nextState = {...state,loading:false,error:false,text:'',code:codeBar,founded:false,status:status,data:{},survey:surv};
            }
            resolve(setNewCodeBar(nextState));
        },(err)=>{
            nextState = {...state,loading:false,error:err.error,text:err.msg,code:codeBar,founded:false,status:SCAN_FOUND_ERROR,data:{}};
            resolve(setNewCodeBar(nextState));
        }).catch((err)=>{
            nextState = {...state,loading:false,error:err,text:err,code:codeBar,founded:false,status:SCAN_FOUND_ERROR,data:{}};
            resolve(setNewCodeBar(nextState));
        });
...
export async function editContainer(dispatch,state,table,value,full) {
    return new Promise(resolve => {
        ...
        if(full){
            ...
            switch(table){
                case 'bl': container ={...state.container,bl_id:value};break;
                case 'fl': container ={...state.container,fl_id:value};break;
                case 'rm': container ={...state.container,rm_id:value};break;
            }
        }
...
export async function saveSurveyAfterScanning(db,dispatch,dbState,state,user){
    return new Promise(resolve=>{
        let requiredField = state.defFields.filter((item)=>{return item.required==='Y'});
        let nextState;
        let errorField=[];
        let survey;
        if(state.code ===null){
            ...
            requiredField.map((item)=>{
                if(item.field_name !== 'fn_std' && state.survey[item.field_name].length ===0){
                    errorField.push(item.ml_heading.replace(/\n|\r|(\n\r)/g,' ').toUpperCase());
                }

            });
        if(errorField.length){
            nextState = {...state,loading:false,error:true,text:'les champs suivants sont requis :'+errorField.join(',')};
            resolve(setNewCodeBar(nextState));
        }else{
            let date = moment(new Date()).format("YYYY-MM-DD HH:mm:ss");
            survey = {...state.survey,date_updated:'', date_scan:date, time_scan:date, scanned_by:user};
            insertDataToTable(db,'ta_surv',Object.keys(survey),[Object.values(survey)]).then(res=>{

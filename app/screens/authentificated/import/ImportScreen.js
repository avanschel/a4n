        ...
        Network.getNetworkStateAsync().then( res=>{
            if(res.isConnected){
                Alert.alert("Import",importWarning+'  thank you for confirming',
                    [
                        {text: 'OK', onPress: () => { this.startImport()}},
                        {text: 'Cancel', onPress: () => { }},
                    ] )
            }else{
...
const mapDispatchToProps = (dispatch) => {
    return {
        importData: async (db, server,state) => {return await dispatch(importData(db, server,dispatch,state))},
        exportData: async (db, server,state) => {return await dispatch(exportData(db, server,dispatch,state))},

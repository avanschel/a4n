...
import {checkBlFlRm, getTableDefTa, onChangeSurvey, onSetSCanState} from "../../../api/scan";
...
class ScanningFormScreen extends React.Component {
    ...
    constructor(props){
        ...
        this.props.getDefFields(this.props.database.db,this.props.scanStatus).then(res=>{
        });
        asyncRetrieveAssetsLimit(this.props.database.db,'bl',null,null,null,null,null).then(bl=>{
            asyncRetrieveAssetsLimit(this.props.database.db,'fl',null,null,null,null,null).then(fl=>{
                asyncRetrieveAssetsLimit(this.props.database.db,'rm',null,null,null,null,null).then(rm=>{
                    asyncRetrieveAssetsLimit(this.props.database.db,'fnstd',null,null,null,null,null).then(fnstd=>{
                        this.blData=bl;
                        ...
                        this.fnstdData=fnstd;
                        this.filterFnstdData=fnstd;
                    });
                });
            });
        });
    ...
    showMyModal(table){
        switch(table){
            case 'bl':
                ...
                this.setState({tableModal:'bl',showMyModal:true,titleModal:'Choisir un bâtiment',dataModal:this.filterBlData});
                break;

            case 'fl':
                this.filterFl(this.props.scanStatus.survey.fl_id);
                this.setState({tableModal:'fl',showMyModal:true,titleModal:'Choisir un étage',dataModal:this.filterFlData});
                break;

            case 'rm':
                this.filterFl(this.props.scanStatus.survey.rm_id);
                this.setState({tableModal:'rm',showMyModal:true,titleModal:'Choisir une pièce',dataModal:this.filterRmData});
                break;
            case 'fnstd':
                this.setState({tableModal:'fnstd',showMyModal:true,titleModal:'Choisir un standard de bien',dataModal:this.filterFnstdData});
                break;
        }
    }
    ...
    filterBl(value){
        this.filterBlData = (value.length)?this.blData.filter(item=>{ return (item.bl_id.startsWith(value))}):this.blData;
        if(value.length){
            this.filterFl('');
            this.filterRm('');
        }
    ...
    filterFnStd(value){
        this.filterFnstdData = (value.length)?this.fnstdData.filter(item=>{ return (item.fn_std.startsWith(value))}):this.fnstdData;
        if(value.length){
            this.filterFl('');
            this.filterRm('');
        }
    ...
    filterFl(value){
        if(this.props.scanStatus.survey.bl_id.length){
            if(value.length){
                this.filterFlData = this.flData.filter(item=> {return (item.bl_id.startsWith(this.props.scanStatus.survey.bl_id) && item.fl_id.startsWith(value))});
            }else{
                this.filterFlData =  this.flData.filter(item=> {return (item.bl_id.startsWith(this.props.scanStatus.survey.bl_id))});
            }
        }else{
            if(value.length){
                this.filterFlData = this.flData.filter(item=> {return item.fl_id.startsWith(value)});
            }else{
    ...
    filterRm(value){
        if(this.props.scanStatus.survey.bl_id.length){
            ...
            if(this.props.scanStatus.survey.fl_id.length) {
                //bl fl rm fill
                if(value.length){
                    this.filterRmData = this.rmData.filter(item=> {return (item.bl_id.startsWith(this.props.scanStatus.survey.bl_id) && item.fl_id.startsWith(this.props.scanStatus.survey.fl_id) && item.rm_id.startsWith(value))});
                }else{
                    this.filterRmData =  this.rmData.filter(item=> {return (item.bl_id.startsWith(this.props.scanStatus.survey.bl_id) && item.fl_id.startsWith(this.props.scanStatus.survey.fl_id))});
                }
            }else{
                //bl and rm fill
                if(value.length){
                    this.filterRmData = this.rmData.filter(item=> {return (item.bl_id.startsWith(this.props.scanStatus.survey.bl_id) && item.rm_id.startsWith(value))});
                }else{
                    this.filterRmData =  this.rmData.filter(item=> {return (item.bl_id.startsWith(this.props.scanStatus.survey.bl_id))});
                }
            }
        }else{
            //bl id fill and fl id fill
            if(this.props.scanStatus.survey.fl_id.length) {
                //bl fl rm fill
                if(value.length){
                    this.filterRmData = this.rmData.filter(item=> {return (item.fl_id.startsWith(this.props.scanStatus.survey.fl_id) && item.rm_id.startsWith(value))});
                }else{
                    this.filterRmData =  this.rmData.filter(item=> {return (item.fl_id.startsWith(this.props.scanStatus.survey.fl_id))});
                }
            }else{
                //bl and rm fill
                if(value.length){
                    this.filterRmData = this.rmData.filter(item=> {return (item.rm_id.startsWith(value))});
                }else{
                    this.filterRmData =  this.rmData.filter(item=> {return true});
                }
            }
        }
    ...
    selectValuesField(defField){
        if(defField.ref_table.length >0) {
            ...
            if(defField.field_name === 'rm_id' || defField.field_name === 'bl_id' ||defField.field_name === 'fl_id'){
                ...
                return (
                    <View style={styles.textInput}>
                        <TextInput style={styles.formInput} onBlur={(input) =>{this.onBlur(defField.field_name.replace('_id',''))}} value={this.props.scanStatus.survey[defField.field_name]} onChangeText={(input) => { this.onFieldChange(defField.field_name,input) }} />
                ...
                return (
                    <View style={styles.textInput}>
                        <TextInput style={styles.formInput}  onBlur={(input) =>{this.onBlowBlur(defField.ref_table,defField.field_name)}} value={this.props.scanStatus.survey[defField.field_name]} onChangeText={(input) => { this.onFieldChange(defField.field_name,input) }} />
                        <AntDesign name={'search1'} style={styles.textInputButton} onPress={() => {
                            this.showMyModal(defField.ref_table)
                        }}/>
                    </View>
            ...
            return (
                <View style={styles.textInput}>
                    <TextInput style={styles.formInput} value={this.props.scanStatus.survey[defField.field_name]} onChangeText={(input) => { this.onFieldChange(defField.field_name,input) }} />
                </View>
            )
        }
    }
    readOnlyField(defField){
            if(defField.readonly==='Y'){
                return ( <Text style={styles.formInputReadOnly} >{(defField.field_name!=='ta_id')?this.props.scanStatus.data[defField.field_name]:'AUTO'} </Text>)
            }else{
    ...
    generateField(defField){
        return(
            <View key={'fie_'+defField.field_name} style={styles.formRow}>
                <Text style={styles.formLabel}>{defField.ml_heading.replace(/\n|\r|(\n\r)/g,' ').toUpperCase()} {(defField.required ==='Y')?'*':''}</Text>
                {
    ...
    showError(){
        ...
        Alert.alert("Vous avez scanné",'Vous devez remplir les champs *',
            [
            {text: 'OK', onPress: () => { this.removeError()}}
            ] );
    }
    removeError(){
    ...
    onBlowBlur(table,field){
        ...
        switch(table){
            case 'fnstd': message ='Le standard de bien renseigné ('+this.props.scanStatus.survey[field]+') n\'existe pas.';break;
        }
    ...
    render(){
        console.log('je scan ici',this.props.scanStatus.error);
        if(this.props.show){
        if(!this.props.manual && this.props.scanStatus.loading){
            ...
            console.log('Im here? ',this.props.scanStatus.loading);
...
const mapDispatchToProps = (dispatch) => {
    return {
        getDefFields: async (db,state) => {return await dispatch(getTableDefTa(dispatch,state,db))},
        changeSurvey:async (state,survey)=>{return await dispatch(onChangeSurvey(state,survey))},
        setNewState:async (state)=>{return await dispatch(onSetSCanState(state)) }
    };
};

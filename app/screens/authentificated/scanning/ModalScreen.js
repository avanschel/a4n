        ...
        return (<View style={{flexDirection:'row'}}>
            {
                this.header.map((header,i)=>{
                    ...
                    return <TouchableWithoutFeedback  key={'vw'+i} onPress={()=>{this.props.press(this.props.table,item);}}><View    style={style}  ><Text style={styles.lineTitle}>{item[header.field_name]}</Text></View></TouchableWithoutFeedback>
                })
            }
        </View>)
    }
            ...
            return(
                <View style={styles.container}>
                    <View style={styles.header} >
                        <Text style={[styles.txt]} onPress={()=>{this.props.press(this.props.table,'title');}}>{this.props.title.toUpperCase()}</Text>
                        <AntDesign name={'close'} style={[styles.icon]} onPress={()=>{this.props.press(this.props.table,'title');}} />
                    </View>
...
const styles = StyleSheet.create({
    ...
    container:{
        position:'absolute',
        top:-60,left:0,right:0,bottom:0,backgroundColor:'#fff',zIndex:10,flex:1,width:'100%'
    },
    header:{
        flexDirection: 'row',
        textAlign:'center',

...
class TableList extends React.Component {
    ...
    getTitle(){
        let elem  = this.props.database.tables.filter((e)=>{ return e.table_name===this.props.database.currentTable});
        if(elem.length){
            elem = elem[0];
            ...
            <View style={{width:this.width,flexDirection:'row'}}>
                {
                    this.props.database.currentHeader.fields.map((item,i)=>{
                       ...
                       return  <View  key={'vw'+i}  style={style}>
                            <Text style={styles.headerTitle}>{item.ml_heading.replace(/\n|\r|(\n\r)/g,' ').toUpperCase()}</Text>
                           <TextInput style={styles.textInput}
                                      onEndEditing={(input)=>{
                                          this.search(item.field_name,input)
                                      }}
                                      onChangeText={(input)=>{this.onChangeText(item.field_name,input)}}>
                           </TextInput>
                        </View>
                    })
                }
        ...
        return (<View style={{flexDirection:'row'}}>
            {
                this.props.database.currentHeader.fields.map((header,i)=>{
                    let style =(i===0)?[styles.row,{width:this.sizeCell}]:[styles.row,{background:'blue',width:this.sizeCell},styles.borderLeft];
                    return <View  key={'vw'+i}  style={style}><Text style={styles.headerTitle}>{item[header.field_name]}</Text></View>
                })
            }
        </View>)
    }
                ...
                return (
                    <View style={styles.container}>
                        ...
                        >
                            <View style={{flexDirection:'column',width:'100%'}}>
                                {this.Header()}
                                <Text style={styles.noElement}>{'No data for '+this.props.database.currentTable} </Text>
                            </View>
                        </ScrollView>
                    </View>
                )
...
const mapDispatchToProps = (dispatch) => {
    return {
        setCurrentData: async (state,currentData,currentPage,reset) => {return await dispatch(setCurrentDataToProps(dispatch,state,currentData,currentPage,reset))}
    };
};
